...
class KMeans:
    ...
    def cluster(self, k=2, tol=1e-2, max_iter=1000, verbose=False):
        ...
        while cur_iter < max_iter:           
            ...
            # Update centroids
            new_centroids, difference = self.update_centroids(k=self.k, data_centroid_labels=self.data_centroid_labels, prev_centroids=prev_centroids)
            ...
            if (np.absolute(difference) < tol).any():
                break
            else: 
                cur_iter += 1
    ...
    def cluster_batch(self, k=2, n_iter=1, verbose=False):
        ...
        for i in range(0, n_iter):
            ...
            values, counts = np.unique(self.get_data_centroid_labels(), return_counts=True)
