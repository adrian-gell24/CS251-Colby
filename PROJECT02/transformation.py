...
class Transformation(analysis.Analysis):
    ...
    def transform(self, C):
        ...
        # print('Shape of Dh.T:', Dh.T.shape)
        MC = (C@Dh.T).T
        MC = MC[:,:-1]
    ...
    def normalize_zscore_together(self):
        ...
        Normalize (update) the data stored in `self.data`.
        ...
        scale = self.scale_matrix(magnitudes=[denominator]*magnitudes) # multiplication adds denominator into list a number of times given by magnitudes
        translate = self.translation_matrix(magnitudes=[-mean]*magnitudes)
        matrix = (scale@translate)
    ...
    def normalize_zscore_separately(self):
        '''Normalize each variable separately by translating its local mean to zero and scaling
        its local standard deviation to one.
        ...
        data_set = self.orig_dataset.select_data(self.data.get_headers())
        means = data_set.mean(axis = 0)
        stds = data_set.std(axis = 0)
        
        denominators = []
        
        for i in stds:
            denominators.append(1/i)
        
        scale = self.scale_matrix(denominators) # multiplication adds denominator into list a number of times given by magnitudes
        translate = self.translation_matrix([-val for val in means])
        matrix = (scale@translate)
    ...
    def whitening(self):
        ...
        #eigen decomposition
        #constructing matrix where each column is an eigenvector of the covariance matrix
        ...
        #constructing diagonal matrix whose diagonal elements are eigenvalues
        #that correspond to the eigenvectors of each column of the eigenvectors matrix made previously
        eigenvalues, _ = np.linalg.eig(cov)
        D = np.diag(eigenvalues)
        print(D.shape)
        
        #constructing whitening matrix
        W = E @ np.linalg.inv(np.sqrt(D)) @ E.T
        W_withones = np.hstack((W, np.ones((W.shape[0], 1))))
        print(W_withones.shape, W_withones)
        
        #constructing whitened data matrix)))
        whitened = self.transform(W_withones)
        print(whitened, whitened.shape)
        
        self.data = data.Data(headers=headers,data=whitened,header2col=header2col)
        return whitened
        
        # covariance = data_set.cov(m = data_set, rowvar = False)
